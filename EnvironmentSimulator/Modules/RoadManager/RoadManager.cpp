...
Position::ReturnCode Position::XYZ2TrackPos(double x3, double y3, double z3, int mode, bool connectedOnly, int roadId, bool check_overlapping_roads, int hintRoad)
{
    // Overall method:
...
//			LOG("No connections from road id %d lane id %d in junction %d", road->GetId(), lane->GetId(), junction->GetId());
		...
		{
			// Map all outgoing roads to their angles with the current one
			std::map<double, int> roads_angles;
			for (int i = 0; i < n_connections; i++)
			{
				LaneRoadLaneConnection lane_road_lane_connection =
				...
				// We want an angle in the [-PI..PI] range
				double heading = atan2(test_pos.GetY() - GetY(), test_pos.GetX() - GetX());
				double heading_diff = fmod(GetHRoadInDrivingDirection() - heading, 2 * M_PI);
				...
				roads_angles.emplace(heading_diff, i);
			}

			if(strategy == Junction::JunctionStrategyType::STRAIGHT) {
				// Finding angle closest to 0°
				// First we find closest over 0°
				auto it = roads_angles.lower_bound(0.);
				if(it == roads_angles.cend()) {
					// No roads over 0?, so we take the last (i.e. highest negative angle)
					connection_idx = roads_angles.rbegin()->second;
				} else {
					// Found closest road over 0°
					// But could the closest under 0° be actually closer?
					auto it_closest = it;
					auto it_prev = std::prev(it);
					if(it_prev != roads_angles.cend()) {
						// Also found one road under 0°
						// Check if it's actually closer
						if(std::abs(it_prev->first) < it->first) {
							it_closest = it_prev;
						}
					}
					connection_idx = it_closest->second;
				}
			} else {
				// We get the n-th road, either starting from left or right
				int n = std::min((int)roads_angles.size(), std::abs(strategy)) - 1;
				if(strategy > 0) {
					auto it = roads_angles.rbegin();
					std::advance(it, n);
					connection_idx = it->second;
				} else {
					auto it = roads_angles.begin();
					std::advance(it, n);
					connection_idx = it->second;
				}
			}
    ...
    for (int i = 0; done == false && i < max_links; i++)
    {
		...
		if (!done)
		{
			...
			if (link == 0 || link->GetElementId() == -1 ||
				static_cast<int>((ret_val = MoveToConnectingRoad(link, contact_point_type, strategy))) < 0)
			{
...
roadmanager::Position::NextJunction roadmanager::Position::GetNextJunction() const
{
	...
	while(true) {
		...
		if (r->GetLink(lt)->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_JUNCTION) {
			next.junction = GetOpenDrive()->GetJunctionById(r->GetLink(lt)->GetElementId());
			break;
		}
...
int Position::GetRoadLaneInfo(double lookahead_distance, RoadLaneInfo* data, LookAheadMode lookAheadMode) const
{
    ...
    {
        if ((int)target.MoveAlongS(lookahead_distance, 0.0) < 0)
        {
            return -1;
        }
    }
...
Position::ReturnCode Position::GetProbeInfo(double lookahead_distance, RoadProbeInfo* data, LookAheadMode lookAheadMode) const
{
    ...
    if (fabs(lookahead_distance) > SMALL_NUMBER)
    {
        retval = target.MoveAlongS(lookahead_distance,
                                   0.0,
                                   Junction::JunctionStrategyType::STRAIGHT,
...
Position::ReturnCode Position::MoveRouteDS(double ds, double* remaining_dist, bool actualDistance)
{
    ...
    {
        MoveAlongS(ds, 0.0, Junction::JunctionStrategyType::STRAIGHT, actualDistance, MoveDirectionMode::ROAD_DIRECTION, false);
    ...
    else
    {
        ...
        MoveAlongS(ds,
                   ...
                   Junction::JunctionStrategyType::STRAIGHT,
                   actualDistance,
                   MoveDirectionMode::ROAD_DIRECTION,
                   false);  // actualDistance = false since ds already adjusted
            ...
            if (nodes.size() > 1)
            {
                ...
                for (int i = (int)nodes.size() - 1; i >= 1; i--)
                {
                    ...
                    connected_pos.MoveAlongS(nodes[i - 1]->fromRoad->GetLength() * 0.33,
                                             ...
                                             Junction::JunctionStrategyType::STRAIGHT,
                                             false,
                                             Position::MoveDirectionMode::ROAD_DIRECTION,
                                             false);
